    ...
    <Card title="访问日志" :padding="0" shadow>
      <Form ref="form" :model="form" inline label-position="right" style="margin-top:20px">
        ...
        <FormItem :label-width="10">
          <Button type="primary" @click="queryLogs">查询访问日志</Button>
        </FormItem>
      </Form>
      <Table
        stripe
...
export default {
    ...
    return {
      ...
      column: [
        ...
        },
        {
          ...
          width: 150,
          render: (h, params) => {
            let date = params.row.access_time
            date = date.replace('T', '  ')
            if (date.indexOf('.') > 0) {
        ...
        },
        {
          ...
          key: 'access_url_path',
          render: (h, params) => {
            let str = params.row.access_url_path
            if (str.length > 30) {
              ...
              h(
                'Button',
                {
                  props: {
                    type: 'primary',
                    size: 'small',
                    to: 'logsdetail/accessLog/' + params.row.mongdb_id
                  },
                  style: {
                    marginRight: '5px'
                  },
  ...
  methods: {
    ...
    showdetail(params) {
      // let url = 'logsdetail/accessLog/' + params.mongdb_id
